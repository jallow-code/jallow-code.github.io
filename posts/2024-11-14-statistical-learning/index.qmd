...
Statistical learning refers to a large collection of tools aimed at understanding the relationship between a set of predictors ($X = X_1, X_2, ..., X_p$) and a quantitative response $Y$. This relationship can be expressed as $Y = f(X) + \epsilon$, where $f$ is an unknown function and $\epsilon$ represents random error. Statistical learning problems are generally classified into two categories: supervised learning and unsupervised learning. Supervised learning involves estimating an output based on one or more inputs. In contrast, unsupervised learning involves inputs but no supervising outputs.
