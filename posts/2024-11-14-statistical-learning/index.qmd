...
pairs.panels(Weekly[1:8], cex.labels = 1)

# total number of times the market had a positive or negative return
table(Weekly$Direction)

# Distribution of the 'Lag' variables
Weekly_long <- Weekly |>
  select("Lag1","Lag2","Lag3","Lag4","Lag5") |>
  pivot_longer(cols = everything(), names_to = "Lag", values_to = "Value")

ggplot(Weekly_long, aes(x = Value)) +
...
Among all the predictors, only `Lag2` is statistically significant. \> c. Compute the confusion matrix and overall fraction of correct predictions. Explain what the confusion matrix is telling you about the types of mistakes made by logistic regression.
